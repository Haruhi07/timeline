...
class ClusteringRLGenerator(ClusteringTimelineGenerator):
    def rl(self,
           args,
           collection,
           env,
        ...
        for c in ranked_clusters:
            ...
            c_sents = self._select_sents_from_cluster(c)
            # print("C", date, len(c_sents), "M", sys_m, "L", sys_l)
            summary, reward = self.summarizer.rl(args, c_sents, env, k=max_summary_sents)
            #print(f'summary: {summary} reward :{reward}')
